...
class parser_lst(object):
  ...
  def struct_lst(self):#{struct:lst file}
    ...
    for i in list(self.content.keys()):
      if structs := name_format.findall(self.content[i]):
        for j in structs:
  ...
  def struct(self):#struct:{offset:name}
    ...
    if name:
      ...
      for struct in name:
        ...
        if content := s_re.search(self.text):
          tmp_dict = {}
            ...
            if offset and uint:
              ...
              if t_name:
                ...
                if (' ' in t_name) or ("=" in t_name) or (";" in t_name) or("\\" in name) or (t_name ==''):
                  WARNING.append(
                      f"Warning:Invalid Pcd name '{t_name}' for Offset {offset} in struct {struct}"
                  )
                elif '[' in t_name:
                  if uint in ['UINT8', 'UINT16', 'UINT32', 'UINT64']:
                    tmp_name = f'{pcdname2_re.findall(t_name)[0]}[0]'
                    ...
                    for i in range(1, pcdname_num):
                      offset += bit
                      tmp_name = f'{pcdname2_re.findall(t_name)[0]}[{i}]'
                      tmp_dict[offset] = tmp_name
                  else:
                    ...
                    line.append(struct)
                    unparse.append(line)
                elif uint in ['UINT8', 'UINT16', 'UINT32', 'UINT64', 'BOOLEAN']:
                  offset = int(offset,10)
      ...
      print(f"ERROR: No struct name found in {self.file}")
      ERRORMSG.append(f"ERROR: No struct name found in {self.file}")
    return info
  ...
  def nameISstruct(self,line,key_dict):
    ...
    if content := s_re.search(self.text):
      ...
      s_size = '0'
      print(f"ERROR: Struct {line[3]} not define mTotalSize in lst file")
      ERRORMSG.append(f"ERROR: Struct {line[3]} not define mTotalSize in lst file")
    size = int(line[0], 10)
    if line[2] != 0:
      for j in range(0, int(line[2], 10)):
        for k in list(key_dict.keys()):
          offset = size  + k
          name = f'{line[1]}[{j}].{key_dict[k]}'
          dict[offset] = name
        size = int(s_size,16)+size
    else:
      for k in list(key_dict.keys()):
        offset = size + k
        name = f'{line[1]}.{key_dict[k]}'
        dict[offset] = name
    return {line[4]: dict}
...
class Config(object):
  ...
  def config_parser(self):
    ...
    if 'DEFAULT_ID:' in read:
      ...
      for i in all_txt[1:]:
        ...
        str_id = ''.join(f'{m}_' for m in ids)
        str_id=str_id[:-1]
        part = [ids]
  ...
  def eval_id(self,id):
    id = id.split("_")
    default_id = id[:len(id)//2]
    platform_id=id[len(id)//2:]
    text = ''.join(
        f"{SECTION}.common.{self.id_name(platform_id[i], 'PLATFORM')}.{self.id_name(default_id[i], 'DEFAULT')},"
    ...
    for x in section[1:]:
      ...
      for each in comment_ori:
        if each != " " and "\x21" > each or each > "\x7E":
          each = '(R)' if bytes(each, 'utf-16') == b'\xff\xfe\xae\x00' else ""
        comment += each
  ...
  def value_parser(self, list1):
    ...
    if list1[first_num + 1] == 'STRING':# parser STRING
      return "{0x0, 0x0}" if list1[-1] == '""' else f'L{list1[-1]}'
      ...
      for i in list2:
        if len(i) % 2 == 0 and len(i) != 2:
          tmp.extend('0x%02x' % (int(f'0x{i}', 16) >> m * 8 & 0xFF)
                     for m in range(0,
                                    len(i) // 2))
        else:
          tmp.append(f'0x{i}')
      line = ''.join(f'{i},' for i in tmp)
      return '{%s}' % line[:-1]
    else:
      return "0x%01x" % int(list1[-1], 16)
...
class GUID(object):
  ...
  def guid_dict(self):
    ...
    for line in guidinfo:
      if list := line.strip().split(' '):
        if len(list)>1:
          guiddict[list[0].upper()]=list[1]
        elif list[0] != ''and len(list)==1:
          print(f"Error: line {line.strip()} can't be parser in {self.guidfile}")
          ERRORMSG.append(
              f"Error: line {line.strip()} can't be parser in {self.guidfile}")
      else:
        print(f"ERROR: No data in {self.guidfile}")
        ERRORMSG.append(f"ERROR: No data in {self.guidfile}")
    return guiddict

  def guid_parser(self,guid):
    if guid.upper() in self.guiddict:
      return self.guiddict[guid.upper()]
    print(f"ERROR: GUID {guid} not found in file {self.guidfile}")
    ERRORMSG.append(f"ERROR: GUID {guid} not found in file {self.guidfile}")
    return guid
        ...
        if c_name in efi_dict:
          ...
          if struct in all_struct:
            ...
            try:
              ...
              WARNING.append(f"Warning: No <HeaderFiles> for struct {struct}")
              title2 = '%s%s|{0}|%s|0xFCD00000{\n <HeaderFiles>\n  %s\n <Packages>\n%s\n}\n' % (PCD_NAME, c_name, struct, '', self.LST.package()[self.lst_dict[lstfile]])
            header_list.append(title2)
          elif struct not in lst._ignore:
            struct_dict ={}
            print(f"ERROR: Struct {struct} can't found in lst file")
            ERRORMSG.append(f"ERROR: Struct {struct} can't found in lst file")
          if c_offset in struct_dict:
            ...
            print(f"ERROR: Can't find offset {c_offset} with struct name {struct}")
            ERRORMSG.append(f"ERROR: Can't find offset {c_offset} with name {struct}")
        else:
          print(f"ERROR: Can't find name {c_name} in lst file")
          ERRORMSG.append(f"ERROR: Can't find name {c_name} in lst file")
...
class write2file(object):
  ...
  def __gen(self,content):
    if type(content) == type(''):
      return content
    elif type(content) in [type([0, 0]), type((0, 0))]:
  ...
  def __readlist(self,list):
    for i in list:
      if type(i) in [type([0, 0]), type((0, 0))]:
        self.__readlist(i)
      elif type(i) == type('') :
        self.text +=i
    return self.text
...
def dtime(start,end,id=None):
  if id:
    print(f"{id} time:{str(end - start)}")
  else:
    print(f"Total time:{str(end - start)[:-7]}")
  ...
  if options.config:
    if options.path:
      if options.output:
        ...
        print(f'Finished, Output files in directory {os.path.abspath(options.output)}')
      else:
