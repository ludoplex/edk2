...
def FileListMacro(FileType):
    return f'{FileType.replace("-", "_").upper()}S'
...
def ListFileMacro(FileType):
    return f"{FileListMacro(FileType)}_LIST"
...
class TargetDescBlock(object):
    ...
    def InitWorker(self, Inputs, Outputs, Commands, Dependencies):
        ...
        self.Target = self.Outputs[0] if self.Outputs else None
...
class FileBuildRule:
    ...
    def __init__(self, Type, Input, Output, Command, ExtraDependency=None):
        ...
        self.ExtraSourceFileList = [] if not ExtraDependency else ExtraDependency
        #
        ...
        self.GenFileListMacro = self.FileListMacro in self.MacroList
        ...
        self.GenIncListFile = self.INC_LIST_MACRO in self.MacroList
        # Check input files
        ...
        # Check output files
        self.DestFileList = []
        self.DestFileList.extend(iter(Output))
    ...
    def __str__(self):
        SourceString = ""
        SourceString += f' {self.SourceFileType} {" ".join(self.SourceFileExtList)} {self.ExtraSourceFileList}'
    ...
    def Instantiate(self, Macros = None):
        ...
        NewRuleObject.DestFileList.extend(
            PathClass(NormPath(File, Macros)) for File in self.DestFileList
        )
...
class BuildRule:
    ...
    def ParseSubSectionHeader(self, LineIndex):
        ...
        for Section in List:
            ...
            Family = TokenList[1].strip().upper() if len(TokenList) > 1 else TAB_COMMON
        ...
        if self._State not in self._StateHandler:
            EdkLogger.error(
                ...
                File=self.RuleFile,
                Line=LineIndex + 1,
                ExtraData=f"Unknown subsection: {self.RuleContent[LineIndex]}",
            )
    ...
    def __getitem__(self, Key):
        ...
        Key = (Type,) + Key[1:] if len(Key) > 1 else (Type, )
    ...
    def _GetBuildRule(self):
        ...
        if RetVal._FileVersion == "":
            ...
            EdkLogger.error(
                "build",
                AUTOGEN_ERROR,
                ExtraData=f"The version number [{RetVal._FileVersion}] of build_rule.txt is less than the version number required by the AutoGen.(the minimum required version number is [{AutoGenReqBuildRuleVerNum}])",
            )
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        print(Br[".c", SUP_MODULE_DXE_DRIVER, "IA32", TAB_COMPILER_MSFT][1])
        print()
        print(Br[".c", SUP_MODULE_DXE_DRIVER, "IA32", "INTEL"][1])
        print()
        print(Br[".c", SUP_MODULE_DXE_DRIVER, "IA32", "GCC"][1])
        print()
        print(Br[".ac", "ACPI_TABLE", "IA32", TAB_COMPILER_MSFT][1])
        print()
        print(Br[".h", "ACPI_TABLE", "IA32", "INTEL"][1])
        print()
        print(Br[".ac", "ACPI_TABLE", "IA32", TAB_COMPILER_MSFT][1])
        print()
        print(Br[".s", SUP_MODULE_SEC, "IPF", "COMMON"][1])
        print()
        print(Br[".s", SUP_MODULE_SEC][1])
